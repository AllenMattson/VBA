Attribute VB_Name = "GetAllFiles_HyperlinkFiles"
Option Explicit
...
Sub GetAllFiles()
    ...
    'added to fill hyperlinks to documents
    Dim LR As Long: LR = Cells(Rows.Count, 1).End(xlUp).Row
    Range("E2").FormulaR1C1 = "=HYPERLINK(CONCATENATE(RC[-4],RC[-3]),LEFT(RC[-3],LEN(RC[-3])-4))"
